...
class ScrollSpy extends Component {
  constructor(props) {
    ...
    this.homeDefaultLink = this.props.router && this.props.router === 'HashRouter' ? '#/' : '/';
    this.hashIdentifier = '#';

    this.scrollTo = this.scrollTo.bind(this);
    this.easeInOutQuad = this.easeInOutQuad.bind(this);
    this.getNavLinkElement = this.getNavLinkElement.bind(this);
    this.getNavToSectionID = this.getNavToSectionID.bind(this);
    this.scrollSection = this.scrollSection.bind(this);
    this.clearOtherNavLinkActiveStyle = this.clearOtherNavLinkActiveStyle.bind(this);
  }
      ...
      navLink.addEventListener('click', (event) => {
        ...
        if (sectionID) {
          let scrollTargetPosition =
            document.getElementById(sectionID).offsetTop - (this.headerBackground ? document.querySelector("div[data-nav='list']").scrollHeight : 0);
